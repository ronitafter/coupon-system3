...
// -------------------------- getAllCompanyCoupons -----------------------------------------
	...
	public List<Coupon> getMyCompanyCoupons(int myCompanyId) throws CouponSystemException {
		if (couponRepository.findCouponsByCompanyId(myCompanyId).isEmpty()) {
			throw new CouponSystemException("getAllCompanyCoupons failed - no coupons found for this company");

		} else {

		}
		return new ArrayList<Coupon>(couponRepository.findCouponsByCompanyId(myCompanyId));
	}
